...
MsgExeNotFound=WARNING - Aicomi files were not found in selected directory. This patch has to be installed directly to the main game directory in order to work properly.%n%nAre you sure that this directory is correct?
...
RunGame=&Run Aicomi
RunWiki=Open Aicomi wiki (FAQ, English manual)
...
TaskDeleteLst=Delete custom listfiles (Attempts to disable hardmods, try if you have issues)
...
jp.MsgIncompatibleModsDetected=警告 - あなたのゲームフォルダで互換性のない改造が検出されました！ あなたは非常に古くなったモッズを使っているか、サマバケ！すくらんぶるがの他のゲームのモッズを使っています。 これを修正するには、古いモッズをすべて削除する必要があります。
jp.MsgExeNotFound=警告 - サマバケ！すくらんぶるが選択されたディレクトリに見つかりませんでした。 正しく動作させるためには、このパッチをサマバケ！すくらんぶるがのメインゲームディレクトリに直接インストールする必要があります。%n%nこのディレクトリが正しいことを確認していますか？
...
jp.RunGame=ゲームを始める
jp.RunWiki=オープン英語サマバケ！すくらんぶるのwiki（FAQ、マニュアル）
...
jp.TaskDeleteLst=カスタムリストファイルを削除します
...
MsgPathTooLong=Your game is in a very deeply nested folder, which can cause serious issues or even prevent the game from starting.%n%nPlease move your game folder to a simpler path like for example "D:\Games\HC" and try to run this patch again.
jp.MsgPathTooLong=ゲームは非常に深くネストされたフォルダーにあります。これにより、深刻な問題が発生したり、ゲームが開始できなくなったりする可能性があります。%n%nゲームフォルダを、たとえば「D:\Games\HC」のような単純なパスに移動して、このパッチをもう一度実行してみてください。

MsgPathNonLatin=There are non-latin characters in the game installation path. This can cause issues with mods and the patch.%n%nMove your game to a simple path like "D:\Games\HC" to avoid issues. It is best to use only a-z, A-Z and 0-9.
jp.MsgPathNonLatin=ゲームのインストールパスに非ラテン文字が含まれています。このため、MODやHFで問題が発生することがあります。%n%n問題を避けるために、ゲームを「D:\Games\HC」のようなシンプルなパスに移動してください。a-z、A-Z、0-9のみを使用するのがベストです。

MsgSteamUpdatesMissing=Your game is missing official updates from Steam. Please go to your Steam Library and update Aicomi before installing this patch.
